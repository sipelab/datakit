...
class AlignedSession:
    """Container for all aligned streams belonging to a session."""
    master_t: np.ndarray                                    # master timeline in seconds
    master_source: str                                      # tag identifying the master clock stream
    camera: Dict[str, CameraTimeline]                       # keys: "meso_metadata", "pupil_metadata"
    streams: Dict[str, AlignedStream]                      # tag -> aligned stream payloads
    report: Dict[str, AlignmentReport]                      # alignment quality reports
    subject: str                                            # subject identifier
    session: str                                            # session identifier
    stream_meta: Dict[str, dict] = field(default_factory=dict)  # per-stream metadata snapshots
    subject_attrs: Dict[str, Any] = field(default_factory=dict)  # subject-level attributes derived from metadata
    session_attrs: Dict[str, Any] = field(default_factory=dict)  # session-level attributes derived from metadata
    _stream_namespace: AttrNamespace | None = field(default=None, init=False, repr=False, compare=False)

    def stream_namespace(self) -> "AttrNamespace":
        """Return stream tags as an attribute-friendly namespace."""

        if self._stream_namespace is None:
            object.__setattr__(self, "_stream_namespace", _build_attr_namespace(self.streams, prefix="stream"))
        return cast(AttrNamespace, self._stream_namespace)


@dataclass(frozen=True)
class ExperimentDataset:
    """Multi-subject, multi-session bundle of :class:`AlignedSession` objects."""
    sessions: Dict[tuple[str, str], AlignedSession]         # (subject, session) -> AlignedSession
    experiment_info: dict                                   # experiment-level metadata
    _subject_namespace: AttrNamespace | None = field(default=None, init=False, repr=False, compare=False)
    
    def get_subjects(self) -> list[str]:
        """Get list of all subjects in the dataset."""
        return sorted(list(set(key[0] for key in self.sessions.keys())))
    
    def get_sessions_for_subject(self, subject: str) -> list[str]:
        """Get list of sessions for a specific subject."""
        return sorted([key[1] for key in self.sessions.keys() if key[0] == subject])
    
    def get_session(self, subject: str, session: str) -> AlignedSession:
        """Get a specific session."""
        return self.sessions[(subject, session)]
    
    def get_all_sessions(self) -> list[AlignedSession]:
        """Get all sessions as a list."""
        return list(self.sessions.values())

    def subjects_namespace(self, *, include_streams: bool = True) -> "AttrNamespace":
        """Return subjects/sessions as an attribute-addressable tree."""

        subject_payload: Dict[str, Dict[str, SessionView]] = {}
        for (subject_id, session_id), aligned in self.sessions.items():
            session_view = SessionView(
                id=session_id,
                subject_id=subject_id,
                aligned=aligned,
                streams=aligned.stream_namespace() if include_streams else AttrNamespace({}),
            )
            subject_payload.setdefault(subject_id, {})[session_id] = session_view

        subject_mapping: Dict[str, SubjectView] = {}
        for subject_id, sessions in subject_payload.items():
            session_mapping = {f"ses_{session_id}": session for session_id, session in sessions.items()}
            subject_mapping[subject_id] = SubjectView(
                id=subject_id,
                sessions=_build_attr_namespace(session_mapping, prefix="session"),
            )

        return _build_attr_namespace(subject_mapping, prefix="subject")

    def as_namespace(self, *, include_streams: bool = True) -> "AttrNamespace":
        """Cached convenience wrapper around :meth:`subjects_namespace`."""

        if include_streams:
            if self._subject_namespace is None:
                object.__setattr__(self, "_subject_namespace", self.subjects_namespace(include_streams=True))
            return cast(AttrNamespace, self._subject_namespace)
        return self.subjects_namespace(include_streams=False)

    @property
    def ns(self) -> "AttrNamespace":
        """Shortcut for :meth:`as_namespace` with streams included."""

        return self.as_namespace()
...
class SessionStreams:
    """Helper grouping all raw streams (timeseries/static/notes) for a session."""

    subject: str
    session: str
    timeseries: Dict[str, LoadedStream]
    static: Dict[str, LoadedStream]
    notes: list[str] = field(default_factory=list)

    def as_dict(self) -> Dict[str, LoadedStream]:
        """Return combined view of timeseries and static streams."""
        merged = dict(self.timeseries)
...
@dataclass
class BuildArtifact:
    """Structured return type for end-to-end dataset builds."""

    manifest: Manifest
    sessions: Dict[Tuple[str, str], SessionStreams]
    dataset: ExperimentDataset
    output_path: Path | None = None

    def summary_rows(self) -> list[tuple[str, str, str, int]]:
        """Quick helper summarizing per-session stream counts."""
        rows: list[tuple[str, str, str, int]] = []
        for (subject, session), streams in sorted(self.sessions.items()):
            rows.append((subject, session, str(len(streams.timeseries)), len(streams.static)))
        return rows


# ---------------------------------------------------------------------------
# Attribute-friendly exploration helpers
# ---------------------------------------------------------------------------

AttrEntry = tuple[str, Any]


class AttrNamespace(SimpleNamespace):
    """Attribute-accessible mapping that plays nicely with tab completion."""

    def __init__(self, entries: Mapping[str, AttrEntry] | None = None) -> None:
        super().__init__()
        self._entries: Dict[str, AttrEntry] = {}
        for attr_name, (original, value) in (entries or {}).items():
            self._entries[attr_name] = (original, value)
            setattr(self, attr_name, value)

    def __repr__(self) -> str:
        keys = ", ".join(self._entries.keys()) or "(empty)"
        return f"<AttrNamespace {keys}>"

    def _ipython_key_completions_(self) -> list[str]:  # pragma: no cover - interactive convenience
        return list(self._entries.keys())

    def items(self) -> Iterable[tuple[str, str, Any]]:
        for attr_name, (original, value) in self._entries.items():
            yield attr_name, original, value

    def keys(self) -> list[str]:
        return list(self._entries.keys())

    def originals(self) -> Dict[str, str]:
        return {attr_name: original for attr_name, (original, _) in self._entries.items()}


def _safe_attr_name(value: str, prefix: str) -> str:
...
def _build_attr_namespace(items: Mapping[str, Any], *, prefix: str) -> AttrNamespace:
    entries: Dict[str, AttrEntry] = {}
    collisions: Counter[str] = Counter()
    for original, value in items.items():
        attr_name = _safe_attr_name(original, prefix)
        if attr_name in entries:
            collisions[attr_name] += 1
            attr_name = f"{attr_name}_{collisions[attr_name]}"
        entries[attr_name] = (original, value)
    return AttrNamespace(entries)
...
class SessionView:
    ...
    streams: AttrNamespace

    def __repr__(self) -> str:  # pragma: no cover - convenience only
        return f"<SessionView {self.subject_id}/{self.id}>"


@dataclass
class SubjectView:
    id: str
    sessions: AttrNamespace

    def __repr__(self) -> str:  # pragma: no cover - convenience only
        return f"<SubjectView {self.id} ({len(list(self.sessions.items()))} sessions)>"